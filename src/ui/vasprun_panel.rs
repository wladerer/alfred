...
/// Vasprun control panel — trajectory, forces, moments, DOS plot, LDOS.
pub fn vasprun_panel_system(
    ...
    egui::Window::new("Vasprun")
        ...
        .show(ctx, |ui| {
            ...
            ui.label(format!("E₀ = {:.6} eV", step.energy.e_0_energy));
            ...
            if ui_state.show_forces {
                ...
                let max_f = step.forces.iter()
                    ...
                    .fold(0.0f64, f64::max);
                ui.label(format!("Max |F| = {:.4} eV/Å", max_f));
            }
            ...
            if has_dos {
                ...
                let max_dos = (0..nedos)
                    .map(|i| {
                        ...
                        if nspins > 1 {
                            m = m.max(dos.total.densities[[1, i]].abs());
                        }
                        m
                    })
                    .fold(0.0f64, f64::max);
                ...
                // For spin-polarized: center line is in the middle, spin 0 goes right, spin 1 goes left
                // For non-polarized: 0 is on the left edge
                let x_center = if spin_polarized {
                    rect.center().x
                } else {
                    rect.left() + 2.0
                };
                ...
                let dos_to_x = |d: f64, spin: usize| -> f32 {
                    ...
                    if spin_polarized {
                        if spin == 0 {
                            // Spin up: right of center
                            x_center + t * (rect.right() - x_center - 2.0)
                        } else {
                            // Spin down: left of center (mirrored)
                ...
                // Draw center line for spin-polarized
                if spin_polarized {
                ...
                let spins_to_plot = if noncollinear { 1 } else { nspins }; // non-collinear: only total
                ...
                for spin in 0..spins_to_plot {
                    ...
                    if points.len() > 1 {
                        // Fill area using egui::Mesh with indexed triangles
                        ...
                        // For each DOS point, add two vertices: one on the curve, one on the zero line
                        for p in &points {
                            mesh.colored_vertex(egui::pos2(zero_x, p.y), fill_color);
                            mesh.colored_vertex(*p, fill_color);
                        }
                        // Triangle strip: for each pair of adjacent points, form two triangles
                        for i in 0..(points.len() - 1) {
                            let base = (i * 2) as u32;
                            // Triangle 1: zero[i], curve[i], zero[i+1]
                            mesh.add_triangle(base, base + 1, base + 2);
                            // Triangle 2: curve[i], curve[i+1], zero[i+1]
                ...
                // Draw selected partial DOS overlay if LDOS is active
                ...
                // Border
                let bc = egui::Color32::from_gray(60);
                ...
                ui.horizontal(|ui| {
                    if spin_polarized {
                        ui.colored_label(dos_color_up, "■ Spin ↑");
                        ui.colored_label(dos_color_dn, "■ Spin ↓");
                    } else {
                        ui.colored_label(dos_color_up, "■ Total DOS");
                        if noncollinear {
                            ui.label(egui::RichText::new("(non-collinear)").small().color(egui::Color32::GRAY));
                        }
                    }
                    if ui_state.show_ldos && has_pdos {
                        ui.colored_label(egui::Color32::from_rgb(255, 200, 50), "■ PDOS");
                    }
                });
