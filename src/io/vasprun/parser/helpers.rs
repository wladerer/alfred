...
impl<'a> VasprunParser<'a> {
    ...
    pub fn fast_skip(&mut self, end_tag: &[u8]) -> super::super::error::Result<()> {
        ...
        let offset = memmem::find(&bytes[pos..], &needle)
            .ok_or_else(|| super::super::error::VasprunError::MissingElement(
    ...
    pub fn read_event_into<'b>(
        &mut self,
        buf: &'b mut Vec<u8>,
    ) -> super::super::error::Result<Event<'b>> {
...
pub fn skip_element(reader: &mut XmlReader, tag: &[u8]) -> super::super::error::Result<()> {
    let mut buf = Vec::new();
    ...
    loop {
        ...
        match reader.read_event_into(&mut buf)? {
            ...
            Event::Eof => {
                return Err(super::super::error::VasprunError::MissingElement(format!(
                    "EOF while skipping <{}>",
...
pub fn read_text(reader: &mut XmlReader) -> super::super::error::Result<String> {
    let mut buf = Vec::new();
    ...
    loop {
        ...
        match reader.read_event_into(&mut buf)? {
            Event::Text(t) => {
                text = t
                    .unescape()
                    .map_err(|e| super::super::error::VasprunError::Other(e.to_string()))?
                    .trim()
                    .to_string();
            }
            Event::End(_) => break,
...
pub fn parse_floats(s: &str) -> super::super::error::Result<Vec<f64>> {
    s.split_whitespace()
        .map(|tok| {
            if tok.chars().all(|c| c == '*') {
                return Ok(f64::NAN);
            }
            tok.parse::<f64>().map_err(|_| super::super::error::VasprunError::ParseValue {
                value: tok.to_string(),
                target: "f64".into(),
...
pub fn parse_f64(s: &str) -> super::super::error::Result<f64> {
    let s = s.trim();
    if s.chars().all(|c| c == '*') {
        return Ok(f64::NAN);
    }
    s.parse::<f64>().map_err(|_| super::super::error::VasprunError::ParseValue {
        value: s.to_string(),
        target: "f64".into(),
        reason: "invalid float".into(),
    })
...
pub fn parse_i64(s: &str) -> super::super::error::Result<i64> {
    s.trim().parse::<i64>().map_err(|_| super::super::error::VasprunError::ParseValue {
        value: s.to_string(),
        target: "i64".into(),
...
pub fn parse_bool(s: &str) -> super::super::error::Result<bool> {
    match s.trim().to_ascii_uppercase().as_str() {
        "T" | "TRUE" | ".TRUE." | "YES" => Ok(true),
        "F" | "FALSE" | ".FALSE." | "NO" => Ok(false),
        other => Err(super::super::error::VasprunError::ParseValue {
...
pub fn parse_v3(s: &str) -> super::super::error::Result<[f64; 3]> {
    let vals = parse_floats(s)?;
    if vals.len() < 3 {
        return Err(super::super::error::VasprunError::ParseValue {
            value: s.to_string(),
            target: "[f64; 3]".into(),
